...
export class OpenAIAuthManager {
  ...
  async logout(projectRoot: string): Promise<boolean> {
    const statePaths = [
      this.getLoginStatePath(projectRoot),
      this.getLegacyLoginStatePath(projectRoot)
    ];
    let removed = false;
  ...
  private getAuthDirectory(projectRoot: string): string {
    return resolve(projectRoot, this.config.runtime.stateDir, "auth", "openai");
  }

  public async readLoginState(
    projectRoot: string
  ...
  private getLegacyLoginStatePath(projectRoot: string): string {
    return resolve(
      projectRoot,
